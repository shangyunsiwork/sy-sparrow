 ...
 * @Last Modified time: 2020-06-12 11:56:56
 */
/* koa 引用  */
...
const MD_HANDLER = Symbol('MD_HANDLER');
...
class Application {
  constructor (opts) {
    ...
    // 应用的中间键总是在框架中间键之后
    this.app.use(compose(this[MD_HANDLER]()));
  ...
  [MD_HANDLER] () {
    const { mdRewrite, middleware, } = this.options;

    mdRewrite.forEach(mr => {
      const { index, func, } = mr;
      if (!MD[index]) return;
      MD.splice(index, 1, func);
    });

    return [ ...MD, ...middleware, ];
  }
